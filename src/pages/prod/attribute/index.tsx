import { type Attribute, attributeApi } from "@/service/api/prod/attribute";
import {
	App,
	Button,
	Form,
...
import AttributeModal, { type AttributeModalType } from "./AttributeModal";
...
export default function AttributePage() {
	...
	const [modalType, setModalType] = useState<AttributeModalType>("add");
	const [editingRecord, setEditingRecord] = useState<Attribute | null>(null);
	...
	const fetchDataSource = useCallback(
		async (page = 1, size = 10, propName?: string) => {
			setLoading(true);
			try {
				const res = await attributeApi.fetchPage({
					current: page,
					size,
					propName,
				});
	...
	const handleDelete = async (propId: number) => {
		try {
			await attributeApi.delete(propId);
	...
	const openModal = (type: AttributeModalType, record?: Attribute) => {
		setModalType(type);
	...
	const columns: ColumnsType<Attribute> = [
		...
		},
		{
			...
			key: "prodPropValues",
			render: (values: Attribute["prodPropValues"]) => (
				<Space wrap>
					{values?.map((item) => (
